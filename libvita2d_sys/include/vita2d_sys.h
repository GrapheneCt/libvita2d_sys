...
//#define RAZOR_SHAREDFB_COUNTER_FIX
...
typedef enum vita2d_io_type {
	VITA2D_IO_TYPE_NORMAL,	//Use sceIo
	VITA2D_IO_TYPE_FIOS2	//Use FIOS2
} vita2d_io_type;

typedef enum vita2d_mem_attrib {
	VITA2D_MEM_ATTRIB_NONE,
	VITA2D_MEM_ATTRIB_SHARED	//Use shared driver memory
} vita2d_mem_attrib;

typedef struct vita2d_init_param {
	unsigned int temp_pool_size;
	unsigned int heap_size;
	unsigned int param_buffer_size;
	unsigned int vdm_ring_buffer_size;
	unsigned int vdm_ring_buffer_attrib;
	unsigned int vertex_ring_buffer_size;
	unsigned int vertex_ring_buffer_attrib;
	unsigned int fragment_ring_buffer_size;
	unsigned int fragment_ring_buffer_attrib;
	unsigned int fragment_usse_ring_buffer_size;
	unsigned int fragment_usse_ring_buffer_attrib;
	unsigned int msaa;
} vita2d_init_param;

typedef struct vita2d_init_param_external {
	unsigned int temp_pool_size;
	unsigned int heap_size;
	SceGxmContext *imm_context;
	SceGxmRenderTarget *render_target;
	void *display_buffer_data[2];
	SceGxmSyncObject *display_buffer_sync[2];
	SceGxmColorSurface *display_color_surface[2];
...
typedef struct vita2d_texture {
	...
	SceGxmDeviceMemInfo *data_mem;
	SceGxmDeviceMemInfo *palette_mem;
	SceGxmDeviceMemInfo *depth_mem;
 ...
 *
 * @return SCE_OK if versions match, VITA2D_SYS_ERROR_VERSION_MISMATCH otherwise.
 */
#if VITA2D_SYS_VERSION < 0135
#define vita2d_check_version(vita2d_version) (SCE_OK);
#else
PRX_INTERFACE int vita2d_check_version(int vita2d_version);
#endif

#if VITA2D_SYS_VERSION < 0143
/**
...
PRX_INTERFACE int vita2d_init();
  ...
  * Initialize vita2d_sys.
  *
  * @param[in] init_param - a pointer to vita2d_init_param structure. Default values will be used for parameters set to 0.
  *
  * @return SCE_OK, <0 on error.
  */
PRX_INTERFACE int vita2d_init(vita2d_init_param *init_param);
#endif

/**
 * Initialize vita2d_sys for use with pre-initialized GXM context.
 *
 * @param[in] init_param - a pointer to vita2d_init_param_external structure.
 *
 * @return SCE_OK, <0 on error.
 */
PRX_INTERFACE int vita2d_init_external(vita2d_init_param_external *init_param);
 ...
 * @return SCE_OK, <0 on error.
 */
PRX_INTERFACE int vita2d_wait_rendering_done();
 ...
 * @return SCE_OK, <0 on error.
 */
PRX_INTERFACE int vita2d_display_set_resolution(int hRes, int vRes);
 ...
 * @return SCE_OK, <0 on error.
 */
PRX_INTERFACE int vita2d_display_set_max_resolution(int hRes, int vRes);
...
/**
 * Set heap type for texture memory.
 *
 * @param[in] type - one of ::SceGxmDeviceHeapId
 *
 */
PRX_INTERFACE void vita2d_texture_set_heap_type(SceGxmDeviceHeapId type);

/**
 * Get heap type for texture memory.
 *
 * @return heap type, one of ::SceGxmDeviceHeapId.
 */
PRX_INTERFACE SceGxmDeviceHeapId vita2d_texture_get_heap_type();

...
vita2d_pvf *generic_vita2d_load_system_pvf(int numFonts, const vita2d_system_pvf_config *configs, float hSize, float vSize, int shared)
{
	...
	ScePvf_t_fontIndex index;
	ScePvf_t_fontId handle;

	for (i = 0; i < numFonts; i++) {
		ScePvf_t_fontStyleInfo style;
...
vita2d_pvf *vita2d_load_custom_pvf(const char *path, float hSize, float vSize)
{
	...
	ScePvf_t_fontId font_handle = scePvfOpenUserFile(font->lib_handle, (char *)path, 1, &error);
	if (error != 0) {
...
vita2d_pvf *vita2d_load_custom_pvf_buffer(void* buf, SceSize bufSize, float hSize, float vSize)
{
	...
	ScePvf_t_fontId font_handle = scePvfOpenUserMemory(font->lib_handle, buf, bufSize, &error);
	if (error != 0) {
...
ScePvf_t_fontId get_font_for_character(vita2d_pvf *font, unsigned int character)
{
	ScePvf_t_fontId font_handle = font->font_handle_list->font_handle;
	vita2d_pvf_font_handle *tmp = font->font_handle_list;
...
static int atlas_add_glyph(vita2d_pvf *font, ScePvf_t_fontId font_handle, unsigned int character)
{

...
vita2d_texture *vita2d_load_additional_GXT(vita2d_texture *initial_tex, int texture_index)
{
	...
	void *actual_texture_data = sceGxtGetDataAddress(initial_tex->data_mem->mappedBase);

	ret = sceGxtInitTexture(&texture->gxm_tex, initial_tex->data_mem->mappedBase, actual_texture_data, texture_index);
...
vita2d_texture *vita2d_load_GXT_file_FIOS2(char* mountedFilePath, int texture_index)
{
	...
	ret = sceGxtCheckData(texture->data_mem->mappedBase);
	...
	void *actual_texture_data = sceGxtGetDataAddress(texture->data_mem->mappedBase);

	ret = sceGxtInitTexture(&texture->gxm_tex, texture->data_mem->mappedBase, actual_texture_data, texture_index);
